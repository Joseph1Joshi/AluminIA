...
# --- 3. FUNCIONES DB (INCLUYE BORRADO) ---
...
def borrar_chat_db(chat_id):
    try:
        # Primero borramos los mensajes (por la clave foránea) y luego el chat
...
# --- 5. CSS (INCLUYE ESTILO PARA BOTÓN BORRAR) ---
st.markdown(f"""
    <style>
    ...
    .aluminia-metal {{
        font-weight: 900; font-size: clamp(2rem, 8vw, 4rem); text-align: center;
        background: linear-gradient(to bottom, #cfd8dc, #90a4ae, #ffffff, #546e7a);
        -webkit-background-clip: text; -webkit-text-fill-color: transparent;
        filter: drop-shadow(0 0 10px rgba(16, 185, 129, 0.3));
    }}

    [data-testid="stChatMessage"] {{ background-color: rgba(6, 40, 30, 0.5) !important; border-radius: 20px !important; }}
    
    /* Estilo para los botones de la barra lateral */
    .stButton button {{ border-radius: 10px !important; }}
    
    /* Badge de autor */
    .author-footer {{ position: fixed; bottom: 15px; right: 20px; color: #10b981; font-size: 0.7rem; }}
    </style>
...
if st.session_state.user is None:
    ...
    st.markdown("<p style='text-align: center; color: #10b981; opacity: 0.8;'>Proyecto por Joseph Torifio</p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for c in chats:
        ...
        with col_c:
            if st.button(f"💬 {c['titulo'][:15]}...", key=f"btn_{c['id']}", use_container_width=True):
                st.session_state.chat_id = c['id']
                m_db = obtener_mensajes_del_chat(c['id'])
                st.session_state.messages = [{"role": m["role"], "content": m["content"]} for m in m_db]
                st.rerun()
        with col_b:
            if st.button("🗑️", key=f"del_{c['id']}", help="Borrar chat"):
...
# --- 8. CHAT ---
st.markdown('<div class="aluminia-metal" style="font-size:1.5rem; text-align:left;">ALUMINIA</div>', unsafe_allow_html=True)
...
st.markdown('<div class="author-footer">de Joseph Torifio</div>', unsafe_allow_html=True)
